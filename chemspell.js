...
var bgcolors = ["#CEDCE0", "#E0CEC8", "#D3E6CC", "#DECFE3", "#FBFCE6", "#00FFFFFF"]
var table;
var number;
var symbol;
var symbols;
...
function color() {
  ...
  if ((Math.floor(Math.random() * 100) === 1) || trick === "rick") {
    ...
    if (getColor !== undefined) {
      ...
      usedColor = colors[colorNum];
    }
    if (bgcolor !== undefined) {
      usedbgColor = bgcolor;
    } else if (bghex !== "#undefined") {
      usedbgColor = bghex;
    } else if (usedbgColor === "transparent") { // Handle transparent background
      document.getElementById("color").innerHTML = "<style>.tableColor {background-color: " + usedColor + "; opacity: 0.7;} body, html {background-color: transparent;}</style>";
      return;
    } else {
      usedbgColor = bgcolors[colorNum];
    }
